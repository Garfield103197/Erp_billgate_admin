...
export class NewsService {
  ...
  getListnews(pageNumber: number, pageSize: number){
    return this.http.get(`api/news?pageNumber=${pageNumber}&pageSize=${pageSize}`).pipe(map((res: any) => res));
  }
  createNews(data: any){
  ...
  deleteNews(newsId){
    return this.http.delete(`api/news/${newsId}`).pipe(map((res: any) => res));
  }
}
